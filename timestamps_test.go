...
func Test_requestLogger(t *testing.T) {
	...
	if logBuffer.String() != expectedLogString {
		t.Errorf("Error. Expected output = %v / Actual output = %v", expectedLogString, logBuffer.String())
	} else if rr.Code != expectedStatusCode {
		t.Errorf("Error. Expected output = %v / Actual output = %v", expectedStatusCode, rr.Code)
	}
}
